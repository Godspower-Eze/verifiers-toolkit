  ...
  async generate(
    ...
    try {
      ...
      try {
        ...
        if (stdout.includes('assert len(public_inputs) > 0')) {
          return {
            success: false,
            error: 'Your circuit has no public inputs. Add at least one `pub` parameter to your `main` function (e.g. `fn main(x: pub Field)`).',
          };
        }

        if (stdout.includes('invalid public inputs offset')) {
          return {
            success: false,
            error: 'Invalid or corrupt VK — the public inputs offset could not be parsed. Ensure you are using a VK generated by `bb write_vk`.',
          };
        }

        const msg = isExecError(err) ? (err.stderr ?? err.stdout ?? String(err)) : String(err);
